-- ============================================================================
-- The register archive: fifteen days between "delete" and gone
--
...
-- `purge_expired_classic_registers()` would make the wall clock exact; the
...
comment on column public.classic_registers.archived_at is
...
create index if not exists classic_registers_archived_idx
  on public.classic_registers (archived_at)
...
create or replace function public.classic_register_archive_days()
returns integer
language sql
immutable
as $$ select 15 $$;
...
create or replace function public.purge_expired_classic_registers()
returns integer
language plpgsql
volatile
security definer
...
declare
  _gone integer;
begin
  delete from public.classic_registers
   where archived_at is not null
     and archived_at < now() - make_interval(days => public.classic_register_archive_days());
...
create or replace function public.archive_classic_register(_register_id uuid)
returns timestamptz
language plpgsql
volatile
security definer
...
begin
  ...
  update public.classic_registers
  ...
  return _when + make_interval(days => public.classic_register_archive_days());
end;
$$;
...
create or replace function public.restore_classic_register(_register_id uuid)
returns void
language plpgsql
volatile
security definer
...
begin
  ...
  if _rows = 0 then
    raise exception 'That register is not in the archive, or its % days are already up',
      public.classic_register_archive_days();
  end if;
end;
$$;
...
create or replace function public.classic_register_archive()
returns table (
  id             uuid,
  name           text,
  slug           text,
...
as $$
begin
  perform public.purge_expired_classic_registers();

  return query
    select
      r.id, r.name, r.slug, d.name, s.name,
      r.archived_at,
      r.archived_at + make_interval(days => public.classic_register_archive_days()),
      ...
      left join public.classic_register_stores st on st.register_id = r.id
     where r.archived_at is not null
       and public.is_classic_register_owner(auth.uid(), r.id)
...
drop policy if exists "owners delete their archived register" on public.classic_registers;
create policy "owners delete their archived register"
  on public.classic_registers for delete to authenticated
  using (public.is_classic_register_owner(auth.uid(), id) and archived_at is not null);
...
-- away for real by create_classic_register() below.
       ...
       select 1 from public.classic_registers r
        where r.deanery_id = _deanery_id
          ...
          and (r.archived_at is null
               or r.archived_at >= now() - make_interval(days => public.classic_register_archive_days()))
     )
   order by s.name;
$$;
...
revoke all on function public.purge_expired_classic_registers()        from public, anon, authenticated;
revoke all on function public.classic_register_archive_days()          from public, anon;
revoke all on function public.archive_classic_register(uuid)           from public, anon;
revoke all on function public.restore_classic_register(uuid)           from public, anon;
revoke all on function public.classic_register_archive()               from public, anon;

grant execute on function public.classic_register_archive_days()       to authenticated;
grant execute on function public.archive_classic_register(uuid)        to authenticated;
grant execute on function public.restore_classic_register(uuid)        to authenticated;
grant execute on function public.classic_register_archive()            to authenticated;
grant execute on function public.purge_expired_classic_registers()     to service_role;
