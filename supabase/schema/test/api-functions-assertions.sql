...
begin
  ...
  perform public.register_enrol_trainee(
    'a5000000-0000-4000-8000-0000000000e1', 'Ent Beta', 'beta@example.invalid', '');
  select data into _ent from public.register_stores
   where register_id = (select id from public.registers where slug='mersey-ent');
...
begin;
do $$
declare _r jsonb := public.register_enrol_trainee(
...
insert into public.register_attendees (id, session_id, name, email, checked_in_at)
...
declare _r record;
begin
  select * into _r from public.register_record_feedback(
  ...
  if not (select feedback_completed from public.register_attendees
  ...
  if exists (
    select 1 from public.register_feedback f
  ...
  if (select count(*) from information_schema.columns
       where table_schema='public' and table_name='register_feedback'
         and column_name in ('attendee_id','name','email','user_id')) <> 0 then
    raise exception 'FAIL 5: register_feedback has a column identifying a person';
  end if;
...
begin
  ...
  if (select count(*) from public.register_feedback
...
begin
  ...
  loop
    foreach _fn in array array[
      'register_enrol_trainee(uuid, text, text, text)',
      'register_record_feedback(uuid, text, integer, jsonb, text)'
    ]
    loop
