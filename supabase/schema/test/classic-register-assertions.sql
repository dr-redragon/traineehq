...
declare _id uuid;
begin
  _id := public.create_classic_register('d0000000-0000-4000-8000-000000000001',
                                '50000000-0000-4000-8000-000000000001', null);

  if _id is null then
    raise exception 'FAIL 1: create_classic_register returned null';
  end if;

  if not public.is_classic_register_owner('11111111-0000-4000-8000-000000000002', _id) then
    raise exception 'FAIL 1: creator is not the owner';
  end if;

  if not exists (select 1 from public.classic_register_stores where register_id = _id) then
    raise exception 'FAIL 1: no classic_register_stores row was created';
  end if;

  if (select name from public.classic_registers where id = _id) <> 'Mersey · Otolaryngology' then
    raise exception 'FAIL 1: unexpected default name %',
      (select name from public.classic_registers where id = _id);
  end if;

  if (select slug from public.classic_registers where id = _id) <> 'mersey-ent' then
    raise exception 'FAIL 1: unexpected slug %',
      (select slug from public.classic_registers where id = _id);
  end if;
...
do $$
begin
  perform public.create_classic_register('d0000000-0000-4000-8000-000000000001',
                                 '50000000-0000-4000-8000-000000000001', null);
...
do $$
begin
  perform public.create_classic_register('d0000000-0000-4000-8000-000000000002',
...
do $$
begin
  if (select count(*) from public.classic_register_stores) <> 0 then
...
declare _row record;
begin
  select * into _row from public.classic_register_directory() limit 1;
...
declare _req uuid;
begin
  _req := public.request_classic_register_access(
    (select id from public.classic_registers where slug = 'mersey-ent'), 'I organise the ENT teaching day');

  if _req is null then
    raise exception 'FAIL 7: request_classic_register_access returned null';
  end if;
...
--    request_classic_register_access() refuses anyone who is already a member, so this
...
do $$
begin
  perform public.decide_classic_register_access(
    (select id from public.classic_register_access_requests where status = 'pending' limit 1), true, null);
...
do $$
begin
  if not public.is_classic_register_member(
       '11111111-0000-4000-8000-000000000003',
...
do $$
begin
  perform public.request_classic_register_access(
    (select id from public.classic_registers where slug = 'mersey-ent'), null);
...
begin
  perform set_config(
    'test.req_id',
    (select id::text from public.classic_register_access_requests where status = 'pending' limit 1),
    true);
end $$;
...
do $$
begin
  if (select count(*) from public.classic_register_access_requests) <> 0 then
  ...
  perform public.decide_classic_register_access(current_setting('test.req_id')::uuid, true, null);
...
begin
  perform public.decide_classic_register_access(
    (select id from public.classic_register_access_requests where status = 'pending' limit 1),
...
do $$
begin
  if (select count(*) from public.classic_register_stores) <> 1 then
    raise exception 'FAIL 11: approved editor still cannot read the register';
  end if;

  if (select role from public.classic_register_members
...
-- ----------------------------------------------------------------------------
-- 12. save_classic_register honours the version guard
-- ----------------------------------------------------------------------------
begin;
set local role authenticated;
set local test.uid = '11111111-0000-4000-8000-000000000003';
...
do $$
declare
  _rid uuid := (select id from public.classic_registers where slug = 'mersey-ent');
  _v   bigint;
begin
  _v := public.save_classic_register(_rid, '{"trainees":[{"id":"a1","name":"A Trainee"}]}'::jsonb, 1);

  if _v <> 2 then
    raise exception 'FAIL 12: expected version 2, got %', _v;
  end if;

  if (select data -> 'trainees' -> 0 ->> 'name' from public.classic_register_stores where register_id = _rid)
     <> 'A Trainee' then
  ...
  begin
    perform public.save_classic_register(_rid, '{"trainees":[]}'::jsonb, 1);
...
do $$
begin
  perform public.save_classic_register(
    (select id from public.classic_registers where slug = 'mersey-ent'), '{}'::jsonb, 2);
...
-- ----------------------------------------------------------------------------
-- 14. classic_register_stores cannot be written directly, bypassing the version guard
-- ----------------------------------------------------------------------------
begin;
set local role authenticated;
set local test.uid = '11111111-0000-4000-8000-000000000003';
...
begin
  ...
  -- and either way save_classic_register()'s version guard cannot be sidestepped.
  begin
  ...
  if (select data ? 'hacked' from public.classic_register_stores
       where register_id = (select id from public.classic_registers where slug = 'mersey-ent')) then
    raise exception 'FAIL 14: a direct update bypassed save_classic_register';
  end if;
  raise notice 'ok 14  direct writes to classic_register_stores change nothing';
end $$;
rollback;
...
begin
  begin
    perform public.remove_classic_register_member(_rid, '11111111-0000-4000-8000-000000000002');
  ...
  perform public.remove_classic_register_member(_rid, '11111111-0000-4000-8000-000000000003');
  if public.is_classic_register_member('11111111-0000-4000-8000-000000000003', _rid) then
...
do $$
begin
  perform public.remove_classic_register_member(
    (select id from public.classic_registers where slug = 'mersey-ent'),
    '11111111-0000-4000-8000-000000000002');
...
-- ----------------------------------------------------------------------------
-- 17. can_create_classic_register: an editor may start their own register
-- ----------------------------------------------------------------------------
begin;
set local role authenticated;
set local test.uid = '11111111-0000-4000-8000-000000000003';
...
do $$
begin
  if not public.can_create_classic_register('11111111-0000-4000-8000-000000000003') then
    raise exception 'FAIL 17: an existing member cannot create a register';
  end if;

  if public.can_create_classic_register('11111111-0000-4000-8000-000000000004') then
...
begin
  begin
    perform public.classic_register_directory();
    raise exception 'FAIL 18: anon can call classic_register_directory()';
  exception when insufficient_privilege then
    raise notice 'ok 18  anon cannot call classic_register_directory()';
  end;
end $$;
rollback;
...
begin
  begin
    select count(*) into _n from public.classic_registers;
  exception when insufficient_privilege then
    _n := 0;
  end;
  if _n <> 0 then raise exception 'FAIL 18b: anon read % classic_registers', _n; end if;
  raise notice 'ok 18b anon reads no classic_registers';
end $$;
rollback;
...
begin
  begin
    select count(*) into _n from public.classic_register_stores;
  exception when insufficient_privilege then
    _n := 0;
  end;
...
begin
  ...
  raise notice 'ok 19  RLS enabled and forced on all four surviving tables';
end $$;
