...
-- ----------------------------------------------------------------------------
-- Fixture: a second register, so "leaks across classic_registers" is testable at all
-- ----------------------------------------------------------------------------
begin;

insert into public.classic_registers (id, deanery_id, specialty_id, name, slug)
...
insert into public.classic_register_stores (register_id, data) values
...
insert into public.classic_register_sessions (id, register_id, title, session_date, local_id) values
...
do $$
declare
  _ent jsonb := public.classic_register_public_roster('a5000000-0000-4000-8000-0000000000e1');
  _uro jsonb := public.classic_register_public_roster('a5000000-0000-4000-8000-0000000000f1');
begin
...
do $$
declare _r jsonb := public.classic_register_public_roster('a5000000-0000-4000-8000-0000000000e1');
begin
...
do $$
begin
  if public.classic_register_public_roster('00000000-0000-4000-8000-000000000000') is not null then
    raise exception 'FAIL 4: an unknown session returned a roster';
  end if;
  if exists (select 1 from public.classic_register_public_session('00000000-0000-4000-8000-000000000000')) then
...
begin
  foreach _t in array array['classic_register_sessions','classic_register_attendees','classic_register_feedback','classic_register_forms']
  loop
    begin
...
begin
  begin
    perform public.classic_register_resolve_trainee_email(
...
begin;
set local role anon;
do $$ begin perform public.classic_register_record_checkin(
...
declare _att jsonb;
begin
  select data->'attendance' into _att from public.classic_register_stores
...
do $$
begin
  perform public.classic_register_record_checkin('a5000000-0000-4000-8000-0000000000e1', 'uro1', 'ST6');
  perform public.classic_register_record_checkin('a5000000-0000-4000-8000-0000000000e1', 'made-up', 'ST6');
end $$;
reset role;
...
declare
  ...
  _after  bigint;
begin
  select version into _before from public.classic_register_stores where register_id = _rid;

  -- As the trainee's browser would, through the anon door.
  set local role anon;
  perform public.classic_register_record_checkin('a5000000-0000-4000-8000-0000000000e1', 'ent1', '');
  reset role;

  select version into _after from public.classic_register_stores where register_id = _rid;
...
do $$
begin
  if (select count(*) from public.classic_register_sessions) <> 1 then
    raise exception 'FAIL 10: a member saw % sessions, expected only their own',
      (select count(*) from public.classic_register_sessions);
  end if;
  if (select register_id from public.classic_register_sessions)
...
do $$
begin
  if (select count(*) from public.classic_register_sessions) <> 0 then
...
begin
  begin
    insert into public.classic_register_sessions (register_id, title, session_date)
    values ((select id from public.classic_registers where slug = 'mersey-ent'), 'Sneaky', '2026-02-01');
