...
-- Before this, `classic_registers` was unique on specialty_id alone, so ENT could have
...
-- ----------------------------------------------------------------------------
-- 1. Existing classic_registers kept the deanery their specialty implied
-- ----------------------------------------------------------------------------
do $$
begin
  if exists (select 1 from public.classic_registers where deanery_id is null) then
    raise exception 'FAIL 1: a register has no deanery after the backfill';
  end if;

  if (select r.deanery_id from public.classic_registers r where r.slug = 'mersey-ent')
     <> 'd0000000-0000-4000-8000-000000000001' then
    raise exception 'FAIL 1: mersey-ent was not backfilled to Mersey';
  end if;
  raise notice 'ok  1  existing classic_registers kept their deanery';
end $$;
...
begin
  ...
  _id := public.create_classic_register('d0000000-0000-4000-8000-000000000002',
  ...
  if (select count(*) from public.classic_registers
       where specialty_id = '50000000-0000-4000-8000-000000000001') <> 2 then
    raise exception 'FAIL 2: expected two classic_registers on that specialty';
  end if;

  if (select slug from public.classic_registers where id = _id) <> 'wessex-ent' then
    raise exception 'FAIL 2: unexpected slug %', (select slug from public.classic_registers where id = _id);
  end if;
...
do $$
begin
  if (select count(*) from public.classic_register_creatable_deaneries()) <> 2 then
...
declare _d record;
begin
  if (select count(*) from public.classic_register_creatable_deaneries()) <> 1 then
    raise exception 'FAIL 3b: a scoped admin sees % deaneries, expected 1',
      (select count(*) from public.classic_register_creatable_deaneries());
  end if;
  select * into _d from public.classic_register_creatable_deaneries();
...
do $$
begin
  if (select count(*) from public.classic_register_creatable_deaneries()) <> 0 then
...
begin
  select count(*) into _mersey
    from public.classic_register_creatable_specialties('d0000000-0000-4000-8000-000000000001');
  select count(*) into _wessex
    from public.classic_register_creatable_specialties('d0000000-0000-4000-8000-000000000002');

  -- Three specialties exist. Mersey has classic_registers on two of them, Wessex on none.
  if _mersey <> 1 then
  ...
  if not exists (
    select 1 from public.classic_register_creatable_specialties('d0000000-0000-4000-8000-000000000002')
...
do $$
begin
  if (select count(*) from public.classic_register_creatable_specialties(
...
-- classic_registers, because creation goes through create_classic_register().
...
do $$
begin
  if (select deanery_name from public.classic_register_directory()
...
begin
  begin
    perform public.classic_register_creatable_deaneries();
