...
begin
  foreach _t in array array[
    'registers','register_members','register_access_requests','register_stores',
...
declare
  ...
  _want text[][] := array[
    ['registers',                't','f','t','t'],
    ['register_members',         't','t','f','f'],
    ['register_access_requests', 't','f','f','t'],
    ['register_stores',          't','f','f','f'],
    ['register_sessions',        't','f','f','f'],
    ['register_attendees',       't','f','f','f'],
    ['register_feedback',        't','f','f','f'],
    ['register_forms',           't','f','f','f']
  ];
...
begin
  ...
  raise exception 'FAIL 3: a member could still issue an UPDATE on register_stores';
exception when insufficient_privilege then
  raise notice 'ok  3  save_register() is the only way to write the blob';
end $$;
rollback;
