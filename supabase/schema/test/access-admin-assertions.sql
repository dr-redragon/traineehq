...
do $$
begin
  if to_regclass('public.register_invites') is not null then
    raise exception 'FAIL 1: register_invites still exists';
  end if;
  raise notice 'ok  1  register_invites dropped';
end $$;
...
do $$
begin
  if not public.is_register_owner('11111111-0000-4000-8000-000000000002',
                                  (select id from public.registers where slug = 'mersey-ent')) then
    raise exception 'FAIL 2: expected …002 to own mersey-ent';
  end if;
  if not public.is_register_member('11111111-0000-4000-8000-000000000003',
                                   (select id from public.registers where slug = 'mersey-ent')) then
...
do $$
begin
  update public.register_members set role = 'owner'
...
do $$
begin
  perform public.set_register_member_role(
...
begin
  perform public.set_register_member_role(_rid, '11111111-0000-4000-8000-000000000003', 'owner');

  if not public.is_register_owner('11111111-0000-4000-8000-000000000003', _rid) then
    raise exception 'FAIL 6: the editor was not promoted';
  end if;

  perform public.set_register_member_role(_rid, '11111111-0000-4000-8000-000000000002', 'editor');

  if public.is_register_owner('11111111-0000-4000-8000-000000000002', _rid) then
...
begin
  ...
  if not public.is_register_member('11111111-0000-4000-8000-000000000004', _rid) then
...
-- ----------------------------------------------------------------------------
-- 9. register_people answers a member, with names and addresses
-- ----------------------------------------------------------------------------
begin;
set local role authenticated;
set local test.uid = '11111111-0000-4000-8000-000000000003';
...
begin
  select count(*) into _n
    from public.register_people((select id from public.registers where slug = 'mersey-ent'));
  ...
  if not exists (
    select 1 from public.register_people((select id from public.registers where slug = 'mersey-ent'))
     where email = 'trainee.ent@example.com'
  ) then
    raise exception 'FAIL 9: addresses are not being returned';
  end if;

  raise notice 'ok  9  register_people answers a member';
end $$;
commit;
...
do $$
begin
  if (select count(*) from public.register_people(
        (select id from public.registers where slug = 'mersey-ent'))) <> 0 then
    raise exception 'FAIL 10: a non-member read a register''s people';
  end if;
  raise notice 'ok 10  register_people is silent for a non-member';
end $$;
commit;
...
begin
  begin
    perform public.register_people('00000000-0000-4000-8000-000000000000');
    raise exception 'FAIL 11: anon called register_people';
  exception when insufficient_privilege then
    raise notice 'ok 11  anon cannot call register_people';
  end;
end $$;
rollback;
...
begin
  begin
    ...
    raise exception 'FAIL 11b: anon called set_register_member_role';
  exception when insufficient_privilege then
    raise notice 'ok 11b anon cannot call set_register_member_role';
  end;
end $$;
rollback;
