...
// Marks a trainee the register holds no address for. classic-register-api invents these
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: attendee } = await admin
      .from("classic_register_attendees")
    ...
    const { data: register } = await admin
      .from("classic_registers").select("name").eq("id", session.register_id).maybeSingle();
    ...
    console.error("classic-register-certificate error:", error);
