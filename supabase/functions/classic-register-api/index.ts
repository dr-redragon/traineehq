...
// classic_register_record_feedback() purely so it can flip the attendee's
// feedback_completed gate. It is never written to classic_register_feedback, and nothing
...
const feedbackUrl = (sessionId: string) =>
  `${APP_BASE_URL}/classic-registers/feedback?s=${encodeURIComponent(sessionId)}`;
...
async function registerOfSession(db: SupabaseClient, sessionId: string): Promise<string | null> {
  ...
  const { data } = await db.from("classic_register_sessions")
...
async function isMember(db: SupabaseClient, userId: string, registerId: string): Promise<boolean> {
  const { data } = await db.from("classic_register_members")
...
async function templateForm(db: SupabaseClient, registerId: string) {
  const { data } = await db.from("classic_register_forms")
    .select("form").eq("register_id", registerId).maybeSingle();
  if (data?.form) return data.form;

  await db.from("classic_register_forms")
...
async function handleCheckIn(db: SupabaseClient, body: Record<string, unknown>) {
  ...
  const { data: session } = await db.from("classic_register_sessions")
  ...
  let enrolled = false;
  if (!traineeId) {
    const { data: joined, error } = await db.rpc("classic_register_enrol_trainee", {
      _session_id: sessionId, _name: name, _email: typedEmail, _grade: grade ?? "",
    });
    if (error) console.error("classic_register_enrol_trainee failed", error);  // attendee row still stands
    ...
    await db.rpc("classic_register_record_checkin", {
  ...
  try {
    const { data: resolved } = await db.rpc("classic_register_resolve_trainee_email", {
    ...
    console.error("classic_register_resolve_trainee_email failed", err);
  }

  if (!isEmail(email)) return json({ error: "A valid email address is required" }, 400);

  const { data, error } = await db.from("classic_register_attendees")
    .upsert({
...
async function handleSubmitFeedback(db: SupabaseClient, body: Record<string, unknown>) {
  ...
  const { data, error } = await db.rpc("classic_register_record_feedback", {
  ...
  if (error) {
    console.error("classic_register_record_feedback failed", error);
...
async function handleCreateSession(
  ...
  if (localId) {
    const { data: existing } = await db.from("classic_register_sessions")
      .select("id").eq("register_id", registerId).eq("local_id", localId).maybeSingle();

    if (existing) {
      const { data, error } = await db.from("classic_register_sessions")
...
async function handleSessionStatus(db: SupabaseClient, sessionId: string) {
  ...
  const { data: attendees, error } = await db.from("classic_register_attendees")
    .select("id, name, email, grade, checked_in_at, feedback_completed, certificate_sent_at")
    .eq("session_id", sessionId).order("name");
  if (error) return json({ error: error.message }, 500);

  const { count } = await db.from("classic_register_feedback")
...
async function handleEmailFeedbackLink(
  ...
  let q = db.from("classic_register_attendees")
...
async function handleChaseAbsences(
  ...
  const { data: store } = await db.from("classic_register_stores")
  ...
  for (const raw of list) {
    ...
    if (!isEmail(email) && localId) {
      try {
        ...
        if (typeof resolved === "string" && resolved) email = cleanEmail(resolved);
      } catch (err) { console.error("classic_register_resolve_trainee_email failed", err); }
    }
    ...
    if (!checkedIn) {
      ...
      await db.from("classic_register_attendees").update({ checked_in_at: null })
        .eq("session_id", sessionId).eq("email", email);
      marked++;
      continue;
    }

    const { error } = await db.from("classic_register_attendees").upsert({
      session_id: sessionId, name, email, grade,
      checked_in_at: new Date().toISOString(),
    }, { onConflict: "session_id,email" });
...
async function handleSaveForm(
  ...
  if (body.reset === true) {
    ...
    const { error } = await db.from("classic_register_sessions")
  ...
  if (sessionId) {
    ...
    if (alsoTemplate) {
      const { error: tplError } = await db.from("classic_register_forms")
  ...
  const { error } = await db.from("classic_register_forms")
    .upsert({ register_id: registerId, form: cleaned.form, updated_at: new Date().toISOString() },
            { onConflict: "register_id" });
  if (error) return json({ error: error.message }, 500);
...
async function handleResetFeedback(
  ...
  const { error } = await db.from("classic_register_attendees")
...
// `classic-register-certificate` function already drawn — see the note at the top of
