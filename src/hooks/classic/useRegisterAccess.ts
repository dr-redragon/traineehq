...
import type { RegisterRole } from "@/lib/classic/types";
...
export function useRegisterMembers(registerId: string | undefined) {
  ...
  return useQuery({
    queryKey: ["classic-register-members", registerId, user?.id],
...
export function useRegisterRequests(registerId: string | undefined) {
  const { data: user } = useCurrentUser();
  return useQuery({
    queryKey: ["classic-register-requests", registerId, user?.id],
...
export function useRegisterPeople(registerId: string | undefined) {
  const { data: user } = useCurrentUser();
  return useQuery({
    queryKey: ["classic-register-people", registerId, user?.id],
    queryFn: async () => {
...
function useRegisterAccessMutation<TArgs>(fn: (args: TArgs) => Promise<unknown>) {
  ...
  return useMutation({
    mutationFn: fn,
    onSuccess: () => {
      for (const key of ["classic-register-members", "classic-register-requests", "classic-register-people", "classic-register-directory"]) {
